// ── COUNTRY LIST (name, ISO 3166-1 alpha-2, E.164 calling code) ──
// Shared by signup.html and enrol.html for the country dropdown +
// auto-filled phone dial code. Loaded as a plain global (no build
// step on this site) so both pages can just do:
//   window.rbcCountries.forEach(c => ...)
window.rbcCountries = [
  ...
  { name: 'Congo (Republic of the)', iso2: 'CG', dial: '+242' },
  { name: 'Congo (DR)', iso2: 'CD', dial: '+243' },
...
// Populates a <select> with every country and keeps a dial-code
// display element in sync with whichever one is selected. Shared by
// signup.html and enrol.html so both forms behave identically.
window.rbcWireCountryPhone = function (countrySelect, dialCodeBox, defaultIso2) {
  window.rbcCountries.forEach((c) => {
  ...
  if (defaultIso2) countrySelect.value = defaultIso2;

  const updateDial = () => {
    const selected = window.rbcCountries.find((c) => c.iso2 === countrySelect.value);
    dialCodeBox.textContent = selected ? selected.dial : '+—';
  };

  countrySelect.addEventListener('change', updateDial);
  updateDial();
};

// Returns the dial code (e.g. "+250") for whatever the given
// <select> currently has chosen, or '' if nothing valid is selected.
window.rbcDialCodeFor = function (countrySelect) {
  const selected = window.rbcCountries.find((c) => c.iso2 === countrySelect.value);
  return selected ? selected.dial : '';
};
